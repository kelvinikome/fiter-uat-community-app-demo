	...
	mifosX.controllers = _.extend(module, {
		EditChargeController: function (scope, resourceFactory, location, routeParams, dateFilter) {
			...
			resourceFactory.chargeResource.getCharge({chargeId: routeParams.id, template: true}, function (data) {
				...
				scope.chargeType = data.limits && data.limits.length ? 'commission' : 'normal';
			...
			scope.changeChargeType = function() {
				if (this.chargeType == 'normal') {
					this.formData.limits = [];
					this.stopComputingAfterDays = undefined;
				} else if (!this.formData.limits) {
					this.formData.limits = [];
				}
			};
			scope.addLimit = function() {
				if (this.minAmount && this.maxAmount && this.limitAmount && (parseInt(this.maxAmount) > -1 && parseFloat(this.maxAmount) >= parseFloat(this.minAmount))) {
					this.formData.limits.push({
						min: this.minAmount,
						max: this.maxAmount,
						amount: this.limitAmount,
						chargeCalculation: this.chargeCalculation,
						locale: scope.optlang.code
					});
					this.minAmount = this.maxAmount = this.limitAmount = this.chargeCalculation = undefined;
				}
			};
			scope.deleteLimit = function(index) {
			...
			scope.submit = function () {
				...
				if (this.formData.limits) {
					this.formData.limits.map(function (x) { x.locale = scope.optlang.code; });
				}
